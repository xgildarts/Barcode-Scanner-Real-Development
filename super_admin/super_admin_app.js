...
async function handleSetEvent() {
    ...
    if (!getToken()) return Swal.fire({ icon:'error', title:'Unauthorized', text:'You must be logged in.' }).then(()=>{ clearSessionAndRedirect() })
    ...
    try {
        const response = await apiFetch('/super_admin/set_event', { method:'POST', body: JSON.stringify({ event_name: eventName, location: _eventLocation, radius: _eventRadius }) })
...
async function handleShowEventQR() {
    try {
        const response = await apiFetch('/super_admin/get_event_qr')
        const data = await response.json()
...
async function handleRegenerateEventQR() {
    ...
    try {
        const response = await apiFetch('/super_admin/regenerate_event_qr', { method:'POST' })
        const data = await response.json()
