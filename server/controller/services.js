...
async function checkStudentIfExistsInRegistration(barcode) {
    return new Promise((resolve, reject) => {
        db.execute(`SELECT 
                        ...
                        sa.student_middlename,
                        sa.student_lastname,
                        sa.student_email,
                        sa.student_year_level,
                        COALESCE(sa.student_guardian_number, src.student_guardian_number) AS student_guardian_number,
                        sa.student_program,
                        sa.barcode_teacher_serial
                    FROM student_accounts sa
                    LEFT JOIN student_records_regular_class src 
                        ON sa.student_id_number = src.student_id_number
                    WHERE sa.barcode = ?
                    LIMIT 1`, [barcode],
            (err, result) => {
                if (err) { return reject(err) }
